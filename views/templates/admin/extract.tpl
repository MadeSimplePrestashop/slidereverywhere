...
{if $exist}
    ...
    {else}
        {if !$zipclass}
            {l s="So sorry. But we can not load ZipArchive module for extract builder extension. Please extract the zip file in the directory {$extractor_path} "};
    {else}
        <a href="{$extractor_url}"  id="extractor">
            <button type="button"  class="btn btn-default">{l s="Extract builder (unzip)"}</button>
            <img src="{$loader}" alt="" style="display:none" />
        </a>
        {/if}
    {/if}
